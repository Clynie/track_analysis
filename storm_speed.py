...
def plot_storm_speed(speed,ft,lats,lons,monthday,times):
    lats = np.ma.masked_where(lats==0,lats)
    lons = np.ma.masked_where(lons==0,lons)
    ft[:,1:] = np.ma.masked_less(ft[:,1:],0.01)
    speed[:,1:] = np.ma.masked_less(speed[:,1:],0.01)
    colors = ['b','g','r','c','m','y','chocolate','b','g','r','c','m','y']
    linestyles = ['-','-','-','-','-','-','-','--','--','--','--','--','--']
    fig1 = plt.figure(1)
    ax1 = fig1.add_subplot(1,1,1)
    for i in np.arange(ft.shape[0]):
        # Ensure values which were appended with 0 are not included
        if speed[i,-1] == 0:
            vt = (ft[i][0:-2]+ft[i][1:-1]) / 2.
            ax1.plot(vt,speed[i,:-1],linewidth=2,linestyle=linestyles[i],color=colors[i])
        else:
            vt = (ft[i][0:-1]+ft[i][1:]) / 2.
...
def main():
    ...
    for md in monthday:
        for TT in times:
            if not (md == 1207 and TT == 12):
                dataname = '/nfs/a37/scjea/model_runs/Hagupit/data/tctracker_data/{0:04d}_{1:02d}Z'.format(md,TT)
    ...
    for i in np.arange(ft.shape[0]):  #i.e. the number of forecasts (13 for Hagupit)
        k = 0
        for j in np.arange(ft.shape[1] - 1): #i.e. the number of times in forecast
